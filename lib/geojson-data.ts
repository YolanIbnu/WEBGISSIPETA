export type StatusType = "HARA" | "LOKAL" | "INDUSTRI" | "VINIR" | string;
export type CacatKayuType = "NORMAL" | "DORENG" | "BUNCAK" | "GROWONG" | "LAPUK" | string;

export const SORTIMEN_GRADE_OPTIONS = ["AI", "AII", "AIII"];
export const STATUS_OPTIONS: string[] = ["LOKAL", "HARA", "VINIR", "INDUSTRI"];
export const CACAT_KAYU_OPTIONS: string[] = ["NORMAL", "DORENG", "BUNCAK", "GROWONG", "LAPUK"];

export const WOOD_TYPES = ["Jati", "Mahoni", "Pinus", "Sengon", "Meranti", "Kayu Putih"];

export function getWoodTypeColor(woodType: string) {
  switch (woodType) {
...
export interface WoodBlock {
  ...
  volume: number;
  logCount: number;
  grade: string; // Displayed as "Sortimen" in UI, values: AI, AII, AIII
  status: StatusType;
